...
def main(page: ft.Page):
        ...
        with snackbar_lock:
            ...
            def show_and_remove():
                 try:
                    ...
                    page_ref.update()
                    time.sleep(0.5) 
                 except Exception as e_show: logging.error(f"Fehler beim Anzeigen/Schließen der Snackbar: {e_show}", exc_info=True)
                 finally:
                    try:
                        if page_ref.overlay and snackbar in page_ref.overlay:
                             page_ref.overlay.remove(snackbar)
                             page_ref.update()
                    except Exception as e_remove: logging.error(f"Fehler beim Entfernen der Snackbar: {e_remove}", exc_info=True)
                    with snackbar_lock:
    ...
    header = ft.Text("📁 Wähle einen Material-Ordner", size=20, weight="bold")
    listview = ft.ListView(expand=True, spacing=0, padding=5)
    back_button = ft.ElevatedButton("⮜ Zurück", tooltip="Zum übergeordneten Ordner", disabled=True, on_click=lambda e: go_back())
    ocr_button = ft.ElevatedButton("📷 Scan & Select", icon=ft.Icons.CAMERA_ALT_OUTLINED, tooltip="Scannt Bildschirmbereich, sucht Name/Durchmesser...", disabled=True, on_click=lambda e: start_ocr_process_thread())
    ...
    def load_items(paths_to_load: list[str], highlight_path: str | None = None):
        ...
        logging.info(f"{len(dirs)} Ordner und {len(prcs)} .prc-Dateien geladen für Pfad: {path_stack[-1] if path_stack else 'Root Auswahl'}")
            ...
            for control in listview.controls:
                 if isinstance(control, ft.ListTile) and hasattr(control, 'data') and control.data:
                     ...
                     if control_data_norm == norm_highlight_path:
                         ...
                         logging.debug(f"Tile für '{os.path.basename(highlight_path)}' nach Laden gefunden und hervorgehoben.")
                         break
            if not highlight_applied:
                logging.warning(f"Pfad '{highlight_path}' sollte hervorgehoben werden, aber Tile wurde nach Laden nicht gefunden.")
    ...
    def open_folder(folder: str, highlight_path: str | None = None):
        ...
        logging.info(f"Öffne Ordner: {folder}" + (f" (versuche '{os.path.basename(highlight_path)}' hervorzuheben)" if highlight_path else ""))
        ...
        if len(path_stack) == 1:
            ...
            header.value = f"📁 {os.path.basename(folder)}"
            back_button.disabled = True
            ocr_button.disabled = False
        else:
            header.value = f"📁 ...{os.sep}{os.path.basename(folder)}"
        ...
        try:
            ...
            logging.error(f"Unerwarteter Fehler beim Öffnen von Ordner '{os.path.basename(folder)}': {e}", exc_info=True)
    ...
    def copy_prc_path_and_show_dialog(file_path: str):
        ...
        try:
            ...
            for control in listview.controls:
                if isinstance(control, ft.ListTile) and hasattr(control, 'data') and control.data:
                   ...
                   if control_data_norm == norm_file_path:
                       ...
                       break
            if not found_tile:
                logging.warning(f"Tile for {filename} (path: {file_path}) not found in current listview controls for highlighting.")
            else:
            ...
            try:
                ...
                display_path = f"...{os.sep}{path_parts[-2]}{os.sep}{path_parts[-1]}" if len(path_parts) >= 2 else filename
            except Exception:
            ...
            logging.error(f"Unexpected Exception in copy_prc_path_and_show_dialog for path {file_path}: {e}", exc_info=True)
            _show_snackbar_message(page, f"❌ Unerwarteter Fehler: {e}", error=True, duration=4000)
            page.update()
    ...
    def go_back():
        ...
        if path_stack:
            ...
            if len(path_stack) == 1:
                header.value = f"📁 {os.path.basename(parent_folder)}"
                back_button.disabled = True
                ocr_button.disabled = False
                current_material_root_path = parent_folder
            else:
                header.value = f"📁 ...{os.sep}{os.path.basename(parent_folder)}"
                back_button.disabled = False
                ocr_button.disabled = False
    ...
    def select_root(index: int):
        ...
        header.value = f"📁 {os.path.basename(selected_folder)}"
        back_button.disabled = True
        ocr_button.disabled = False
        ...
        try:
            ...
            logging.error(f"Unerwarteter Fehler beim Laden des Root-Ordners '{os.path.basename(selected_folder)}': {e}", exc_info=True)
            _show_snackbar_message(page, f"Unerwarteter Fehler: {e}", error=True, duration=4000)
            current_material_root_path = None
            header.value = "📁 Wähle einen Material-Ordner"
            ocr_button.disabled = True
    ...
    def run_ocr_process(page_ref: ft.Page):
        ...
        if not current_material_root_path:
            ...
            # ocr_button.disabled bleibt True, wenn kein Root-Pfad
            page_ref.update()
            return
        ...
        try:
            ...
            if found_feature_type_str and current_material_root_path:
                ...
                logging.info(f"OCR fand Feature-Typ: '{found_feature_type_str}' (verwendet als: '{feature_type_lower_cleaned}'). OCR-Gesamtergebnis: {ocr_results_local}")
                
                target_prc_path_local = rule_engine.find_prc_path_by_rules(
                    feature_type_lower_cleaned, ocr_results_local, current_material_root_path # Übergebe alle OCR-Ergebnisse
                )
        ...
        if target_prc_path_local:
            try:
                ...
                if current_dir and target_dir and target_dir != current_dir:
                    logging.info(f"Navigation erforderlich: Von '{current_dir}' zu '{target_dir}' für Datei '{filename}'")
                    ...
                    if found_tile_direct:
                         ...
                         try:
                             ...
                             listview.scroll_to(key=target_prc_path_local, duration=500, curve=ft.AnimationCurve.EASE_IN_OUT)
                         except Exception as scroll_err: 
                             logging.warning(f"Fehler beim Scrollen zu Key '{target_prc_path_local}' (direkt): {scroll_err}")
                    else:
                         logging.warning(f"Datei '{filename}' sollte im aktuellen Verzeichnis '{current_dir}' sein, aber Tile nicht gefunden.")
                         _show_snackbar_message(page_ref, f"⚠️ Datei '{filename}' nicht in Liste gefunden, obwohl im Ordner erwartet. Pfad wurde kopiert.", duration=4500)
                else:
                ...
                _show_snackbar_message(page_ref, f"❌ Fehler bei Ergebnisverarbeitung: {proc_err}", error=True, duration=4000)
